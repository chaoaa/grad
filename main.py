...
class Trainer(object):
    ...
    def setup_dataset(self):
        self.coco_set = datasets.coco_dataset.CocoDataset(
            ...
            keyword_mask_path = cfg.DATA_LOADER.KEYWORD_MASK_PATH,  # KSC
        )
    ...
    def make_kwargs(self, indices, input_seq, target_seq, keyword_mask,
        ...
        # keyword_mask 与 input/target 截断对齐
        if keyword_mask is not None:
            keyword_mask = keyword_mask[:, 0:max_len].contiguous()
        ...
        kwargs = {
            ...
            'KEYWORD_MASK':          keyword_mask,   # KSC（None 表示未启用）
        }
        return kwargs
    ...
    def forward(self, kwargs):
        if self.rl_stage == False:
            # ============================================================
            # XE 训练阶段：主干 XE Loss + MFR 辅助 Loss + 可选 KSC Loss
            # ============================================================
            logit, scores = self.model(**kwargs)

            target       = kwargs[cfg.PARAM.TARGET_SENT]          # [B*S, L]
            keyword_mask = kwargs.get('KEYWORD_MASK', None)       # [B*S, L] or None

            # --- 主干 XE loss（LabelSmoothing 或 CrossEntropy）---
            loss_xe, _ = self.xe_criterion(logit, target)

            # --- MFR 辅助 CE loss ---
            # scores: raw logits [B*S*L, V]；target pad=-1 用 clamp+valid_mask 处理
            flat_target       = target.reshape(-1)                 # [B*S*L]
            valid_mask        = (flat_target >= 0).float()         # pad 位置置 0
            flat_target_clamp = flat_target.clamp(min=0)           # 避免负数索引

            mfr_ce  = F.cross_entropy(scores, flat_target_clamp, reduction='none')  # [B*S*L]
            mfr_ce  = mfr_ce * valid_mask                          # 屏蔽 pad
            loss_mfr = mfr_ce.sum() / valid_mask.sum().clamp(min=1.0)

            # --- KSC loss（仅当 KSC_ENABLE=True 且 keyword_mask 有效时）---
            ksc_enabled = (cfg.LOSSES.KSC_ENABLE
                           and keyword_mask is not None
                           and keyword_mask.sum().item() > 0)

            if ksc_enabled:
                flat_kw   = keyword_mask.float().reshape(-1)       # [B*S*L]

                # L_KWCE: 对关键词位置施加更强监督的加权 CE
                kw_weight  = 1.0 + cfg.LOSSES.KSC_ALPHA * flat_kw  # [B*S*L]
                loss_kwce  = (mfr_ce * kw_weight * valid_mask).sum() / \
                             (kw_weight * valid_mask).sum().clamp(min=1.0)

                # L_CONS: 主分支与 MFR 分支在关键词位置的 KL 一致性
                # logit 是主分支 log_softmax 输出，exp 还原为概率
                main_logprob = logit.reshape(-1, logit.size(-1)).detach()  # [B*S*L, V]
                main_prob    = main_logprob.exp().clamp(min=1e-8)          # [B*S*L, V]
                mfr_logprob  = F.log_softmax(scores, dim=-1)               # [B*S*L, V]
                # KL(main || mfr) per token
                kl = F.kl_div(mfr_logprob, main_prob, reduction='none').sum(-1)  # [B*S*L]
                loss_cons = (kl * flat_kw * valid_mask).sum() / \
                            (flat_kw * valid_mask).sum().clamp(min=1.0)

                loss_ksc = loss_kwce + cfg.LOSSES.KSC_BETA * loss_cons

                loss = (loss_xe
                        + cfg.LOSSES.MFR_LAMBDA * loss_mfr
                        + cfg.LOSSES.KSC_LAMBDA * loss_ksc)

                loss_info = {
                    'loss_xe':    loss_xe.item(),
                    'loss_mfr':   loss_mfr.item(),
                    'loss_kwce':  loss_kwce.item(),
                    'loss_cons':  loss_cons.item(),
                    'loss_total': loss.item(),
                }
            else:
                # KSC 关闭：退化为原始 XE + MFR
                loss = loss_xe + cfg.LOSSES.MFR_LAMBDA * loss_mfr
                loss_info = {
                    'loss_xe':    loss_xe.item(),
                    'loss_mfr':   loss_mfr.item(),
                    'loss_total': loss.item(),
    ...
    def train(self):
        ...
        for epoch in range(self.load_epoch + 1, cfg.SOLVER.MAX_EPOCH):
            ...
            with tqdm.tqdm(desc='Epoch %d - train' % epoch, unit='it', total=len(self.training_loader)) as pbar:
                for _, (indices, input_seq, target_seq, keyword_mask,
                        gv_feat, att_feats, att_mask) in enumerate(self.training_loader):
                    # data_time.update(time.time() - start)
                    input_seq    = input_seq.cuda()
                    target_seq   = target_seq.cuda()
                    keyword_mask = keyword_mask.cuda()   # KSC
                    gv_feat      = gv_feat.cuda()
                    att_feats    = att_feats.cuda()
                    att_mask     = att_mask.cuda()

                    kwargs = self.make_kwargs(
                        indices, input_seq, target_seq, keyword_mask,
                        gv_feat, att_feats, att_mask
                    )
