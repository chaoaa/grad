...
class DecoderLayer(nn.Module):
    def __init__(self, embed_dim=512, num_heads=8, dropout=0.1, ff_dropout=0.1, use_gx=False):
        ...
        if self.use_gx:
            ...
            # SACF: Semantic-Adaptive Cross-modal Fusion 门控模块
            # 仅当 use_gx=True 时创建；通过 cfg.MODEL.SACF_ENABLE 在 forward 中动态开关
            self.sacf_ctx_proj = nn.Linear(embed_dim, embed_dim)
            self.sacf_gx_proj  = nn.Linear(embed_dim, embed_dim)
            self.sacf_vis_proj = nn.Linear(embed_dim, embed_dim)
            self.sacf_ln       = nn.LayerNorm(embed_dim)
            self.sacf_drop     = nn.Dropout(dropout)
    ...
    def forward(self, gx, x, encoder_out, seq_mask, att_mask=None):
        ...
        # C-MSA: 词嵌入与视觉 token 的跨模态注意力
        short_cut = x

        # ---------- 构造 kv（保留原始逻辑）----------
        if self.use_gx:
            kv = torch.cat([encoder_out, gx.unsqueeze(1)], 1)  # [B, M+1, D]
            if att_mask is not None:
                # 对 mask 末尾补一列 1，对应 gx token
                _att_mask = torch.cat(
                    [att_mask,
                     torch.ones(att_mask.size(0), device=att_mask.device,
                                dtype=att_mask.dtype).unsqueeze(1).unsqueeze(1)],
                    dim=2
                ).long()
            else:
                _att_mask = None
        ...
        # ---------- SACF: 对 kv 进行语义自适应重标定 ----------
        # 当 cfg.MODEL.SACF_ENABLE=True 且 use_gx=True 时生效；否则严格保持原始 kv
        if self.use_gx and cfg.MODEL.SACF_ENABLE:
            # ctx: 当前文本隐藏状态的上下文摘要 [B, 1, D]
            ctx = self.sacf_ctx_proj(x.mean(dim=1, keepdim=True))
            # 融合全局视觉语义
            ctx = ctx + self.sacf_gx_proj(gx).unsqueeze(1)             # [B, 1, D]
            # 门控: 对每个视觉 token 自适应缩放
            gate = torch.sigmoid(self.sacf_vis_proj(kv) + ctx)         # [B, M+1, D]
            kv_adapt = self.sacf_ln(kv + self.sacf_drop(gate * kv))    # [B, M+1, D]
        else:
            kv_adapt = kv
