...
def sample_collate(batch):
    # 训练阶段：dataset 返回 6-tuple
    # (indices, input_seq, target_seq, keyword_seq, gv_feat, att_feats)
    indices, input_seq, target_seq, keyword_seq, gv_feat, att_feats = zip(*batch)
    ...
    # keyword_seq 与 target_seq 完全对齐
    keyword_seq = torch.cat([torch.from_numpy(np.array(b, dtype='int32')) for b in keyword_seq], 0)
    gv_feat    = torch.cat([torch.from_numpy(b) for b in gv_feat], 0)

    # 读取图像的预训练特征时，大小为[L, D]，其中 L 的长度可能不一（如目标特征）
    ...
    # 返回 7-tuple（新增 keyword_seq）
    return indices, input_seq, target_seq, keyword_seq, gv_feat, att_feats, att_mask
...
def load_val(image_ids_path, gv_feat_path, att_feats_folder):
    coco_set = CocoDataset(
        ...
        keyword_mask_path = None,   # 验证/测试阶段不需要 keyword mask
    )
